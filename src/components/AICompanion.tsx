...
export default function AICompanion({ isOpen, onClose, streak }: AICompanionProps) {
  ...
  useEffect(() => {
    ...
    const msg = getMessage(type, todayCount, streak, 0, 0);
    setMessage(msg);
  }, [typeIdx, words]);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center" onClick={onClose}>
      <div className="bg-[#f8fafc] rounded-t-2xl sm:rounded-2xl w-full max-w-md max-h-[85vh] overflow-y-auto pb-6" onClick={e => e.stopPropagation()}>
        {/* Companion Card */}
        <div className="bg-gradient-to-r from-[#4f6ef7] to-[#7c5cfc] p-5 text-white rounded-t-2xl sm:rounded-t-2xl">
          ...
          <p className="text-sm leading-relaxed border-t border-white/20 pt-3">{message}</p>
        </div>

        {/* Today's Words */}
        <div className="bg-white rounded-xl mx-4 -mt-3 p-4 shadow-sm">
            ...
            <div className="max-h-48 overflow-y-auto -mx-1">
              {words.map((w: any) => (
                ...
                >
                  ...
                  <button onClick={(e) => { e.stopPropagation(); speakWord(w.w); }} className="text-sm hover:scale-110 transition-transform">🔊</button>
                </div>
              ))}
            </div>
          )}
        ...
        {selectedWord && (
          <div className="bg-white rounded-xl mx-4 mt-3 p-4 shadow-sm">
            <h3 className="text-lg font-bold text-gray-800 mb-0.5">
              {selectedWord.w} <span className="text-sm font-normal text-gray-400">{selectedWord.p}</span>
            </h3>
            <p className="text-sm text-[var(--primary)] mb-3">{selectedWord.m}</p>
            {selectedWord.ex?.map((ex: any, i: number) => (
              <div key={i} className="mb-2 p-2.5 bg-gray-50 rounded-lg">
